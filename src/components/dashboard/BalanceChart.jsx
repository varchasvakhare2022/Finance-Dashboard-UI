...
function ChartTooltip({ active, payload, label }) {
  ...
  return (
    <div className="rounded-2xl border border-line/80 bg-surface px-4 py-3 shadow-lg backdrop-blur">
      ...
      <p className="mt-2 text-base font-bold text-ink">{formatCurrency(payload[0].value)}</p>
    </div>
  );
}

export function BalanceChart({ data, dateRange, onRangeChange, netFlow }) {
  return (
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm leading-6 text-muted">
            Hover the line to inspect day-level balance changes across the selected date range.
          </p>
        </div>
